"""
Exercício 19 da seção de estrutura de decisão da Python Brasil:
https://wiki.python.org.br/EstruturaDeDecisao

Faça um Programa que leia um número inteiro menor que 1000 e imprima a quantidade de centenas, dezenas e unidades do
mesmo.
Observando os termos no plural a colocação do "e", da vírgula entre outros. Exemplo:
326 = 3 centenas, 2 dezenas e 6 unidades
12 = 1 dezena e 2 unidades Testar com: 326, 300, 100, 320, 310,305, 301, 101, 311, 111, 25, 20, 10, 21, 11, 1, 7 e 16

    >>> decompor_numero(1000)
    'O número precisa ser menor que 1000'
    >>> decompor_numero(-1)
    'O número precisa ser positivo'
    >>> decompor_numero(326)
    '326 = 3 centenas, 2 dezenas e 6 unidades'
    >>> decompor_numero(300)
    '300 = 3 centenas'
    >>> decompor_numero(100)
    '100 = 1 centena'
    >>> decompor_numero(320)
    '320 = 3 centenas e 2 dezenas'
    >>> decompor_numero(310)
    '310 = 3 centenas e 1 dezena'
    >>> decompor_numero(305)
    '305 = 3 centenas e 5 unidades'
    >>> decompor_numero(301)
    '301 = 3 centenas e 1 unidade'
    >>> decompor_numero(311)
    '311 = 3 centenas, 1 dezena e 1 unidade'
    >>> decompor_numero(111)
    '111 = 1 centena, 1 dezena e 1 unidade'
    >>> decompor_numero(101)
    '101 = 1 centena e 1 unidade'
    >>> decompor_numero(25)
    '25 = 2 dezenas e 5 unidades'
    >>> decompor_numero(20)
    '20 = 2 dezenas'
    >>> decompor_numero(21)
    '21 = 2 dezenas e 1 unidade'
    >>> decompor_numero(10)
    '10 = 1 dezena'
    >>> decompor_numero(16)
    '16 = 1 dezena e 6 unidades'
    >>> decompor_numero(1)
    '1 = 1 unidade'
    >>> decompor_numero(7)
    '7 = 7 unidades'

"""


def decompor_numero(numero: int):
    """Escreva aqui em baixo a sua solução"""

    centenas = dezenas = unidade = ''
    partes_numericas = 0

    if numero >= 1000:
        print("'O número precisa ser menor que 1000'")
        return
    elif numero < 0:
        print("'O número precisa ser positivo'")
        return
    else:
        ctn = numero // 100
        dez = (numero - (ctn * 100)) // 10
        un = (numero - ((ctn * 100) + (dez * 10)))
        if numero >= 1:
            if un == 1:
                partes_numericas += 1
                unidade = f'1 unidade'
            elif un > 1:
                partes_numericas += 1
                unidade = f'{un} unidades'
        if numero >= 10:
            if dez > 1:
                partes_numericas += 1
                dezenas = f'{dez} dezenas'
            elif dez == 1:
                partes_numericas += 1
                dezenas = f'{dez} dezena'
        if numero >= 100:
            partes_numericas += 1
            if ctn > 1:
                centenas = f'{ctn} centenas'
            else:
                centenas = f'{ctn} centena'



        if partes_numericas == 0:
            print("O número 0 não possui centena, dezena ou unidade")
        elif partes_numericas == 1:
            print(f"'{numero} = {centenas}{dezenas}{unidade}'")
        elif partes_numericas == 2:
            if len(centenas) == 0:
                print(f"'{numero} = {dezenas} e {unidade}'")
            else:
                print(f"'{numero} = {centenas} e {dezenas}{unidade}'")
        elif partes_numericas == 3:
            print(f"'{numero} = {centenas}, {dezenas} e {unidade}'")
