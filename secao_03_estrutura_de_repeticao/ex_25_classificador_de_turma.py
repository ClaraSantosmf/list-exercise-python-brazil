...
def classifcar_turma(*idades) -> str:
    ...
    a = sum(idades)/len(idades)
    if a <= 25:
        return f'A turma é jovem, pois a média é de {a:.1f} anos'
    if a > 26 and a < 60:
        return f'A turma é adulta, pois a média é de {a:.1f} anos'
    else:
        return f'A turma é idosa, pois a média é de {a:.1f} anos'